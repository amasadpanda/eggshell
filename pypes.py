...
class Pipeline():
  ...
  def __init__(self, funcs=[], eggs=None):
    self.functions = funcs
    self.input = eggs

  def __or__(self, callback):
    if callable(callback):
      return Pipeline(self.functions + [callback], self.input)
    else:
  ...
  async def __acall__(self, egg=None):
    async for i in (egg or self.functions[0]()):
      result = i
      callbacks = self.functions if egg else self.functions[1:]
  ...
  def __call__(self, egg=None):
    self.input = egg or self.input 
